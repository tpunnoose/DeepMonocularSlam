...
class MonocularOdometry(object):
    ...
    def process_next_img(self, visualize=False):
        ...
        if self.initialized:
            ...
            next_pose_cnn = SE3Pose(T[0:3, 0:3], T[0:3,3])

            prev_image_points, prev_3d_points, next_image_points, next_3d_points = self.temporal_matches(self.prev_frame_1, self.next_frame, next_pose_cnn)
    ...
    def temporal_matches(self, prev_frame, next_frame, next_pose_pred, matching_distance=50, point_distance=0.5):
        ...
        for match in all_matches:
            d = np.linalg.norm((next_frame.points_3d[:, match.trainIdx] - next_pose_pred @ prev_frame.points_3d[:, match.queryIdx])[0:3])

            if match.distance < matching_distance and d < point_distance:
                self.matches.append(match)

                prev_image_points.append(prev_frame.keypoints[match.queryIdx].pt)
                prev_3d_points.append(prev_frame.points_3d[:, match.queryIdx])

                next_image_points.append(next_frame.keypoints[match.trainIdx].pt)
                next_3d_points.append(next_frame.points_3d[:, match.trainIdx])

        if self.verbose:
            print("Number of successful temporal matches: ", len(self.matches))
